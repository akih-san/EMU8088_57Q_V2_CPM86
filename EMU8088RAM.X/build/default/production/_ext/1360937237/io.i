...
int seek_disk(void) {
 ...
 if (drives[req_tbl.disk_drive].filep == ((void*)0)) return(-1);
...
int setup_drive(void) {
 ...
 if ( drives[req_tbl.disk_drive].sectors == 0 ) return( -1 );
 if ( req_tbl.disk_sector > drives[req_tbl.disk_drive].sectors ) return( -1 );
 return( 0 );
}
...
void bus_master_operation(void) {
 ...
 read_sram(0xff500, (uint8_t *)&req_tbl, (unsigned int)sizeof(PTRSAV));

 if (req_tbl.UREQ_COM) {
  unimon_console();

  write_sram(0xff500, (uint8_t *)&req_tbl, 2);
 }
 else {
  ...
  write_sram(0xff506, (uint8_t *)&req_tbl.CREQ_COM, 2);
 }
